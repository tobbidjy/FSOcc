...
class DepthwiseSeparableConv3d(nn.Module): # 三维深度可分离卷积
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True): #偏置bias先不考虑
        super(DepthwiseSeparableConv3d, self).__init__()
        # in_channels=embed_dims, out_channels=embed_dims, kernel_size=3, stride=1,padding=1
...
class ShuffleAttention3D(nn.Module):
    ...
    def __init__(self, channel, G,reduction=16):
        super().__init__()
        self.G = G
        self.channel = channel
        self.reduction = reduction
        self.avg_pool = nn.AdaptiveAvgPool3d(1)  # 3D 全局平均池化
        self.gn = nn.GroupNorm(channel // (2 * G), channel // (2 * G))  # GroupNorm 支持 3D 输入
        self.cweight = Parameter(torch.zeros(1, channel // (2 * G), 1, 1, 1))  # 3D 权重
        self.cbias = Parameter(torch.ones(1, channel // (2 * G), 1, 1, 1))  # 3D 偏置
        self.sweight = Parameter(torch.zeros(1, channel // (2 * G), 1, 1, 1))  # 3D 权重
        self.sbias = Parameter(torch.ones(1, channel // (2 * G), 1, 1, 1))  # 3D 偏置
        self.sigmoid = nn.Sigmoid()

    @staticmethod
    def channel_shuffle(x, groups):
        b, c, d, h, w = x.shape
        x = x.reshape(b, groups, -1, d, h, w)  # 按组分组
        x = x.permute(0, 2, 1, 3, 4, 5)  # 调整维度顺序
        x = x.reshape(b, -1, d, h, w)  # 展平
        return x

    def forward(self, x):
        b, c, d, h, w = x.size()  # 输入形状: (batch_size, channels, depth, height, width)
        # 按组分组
        x = x.view(b * self.G, -1, d, h, w)  # (batch_size * G, channels // G, depth, height, width)

        # 通道分割
        x_0, x_1 = x.chunk(2, dim=1)  # 每个组分成两部分

        # 通道注意力
        x_channel = self.avg_pool(x_0)  # 3D 全局平均池化
        x_channel = self.cweight * x_channel + self.cbias  # 加权和偏置
        x_channel = x_0 * self.sigmoid(x_channel)  # 应用 Sigmoid 激活

        # 空间注意力
        x_spatial = self.gn(x_1) # 3D GroupNorm
        x_spatial = self.sweight * x_spatial + self.sbias  # 加权和偏置
        x_spatial = x_1 * self.sigmoid(x_spatial)  # 应用 Sigmoid 激活

        # 沿通道维度拼接
        out = torch.cat([x_channel, x_spatial], dim=1)  # 拼接两部分
        out = out.contiguous().view(b, -1, d, h, w)  # 恢复原始形状
...
class OccLayer(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
        ...
        DSConv = False # 切换深度分离卷积 # 20250303
        ...
        for i in range(conv_num):
            if not DSConv:
                ...
                conv_layer = build_conv_layer(
                        ...
                        padding=1,
                        groups=embed_dims) 
            # # DSConv3d 三维深度可分离卷积 # 20250219
            # conv_layer = DepthwiseSeparableConv3d(in_channels=embed_dims, out_channels=embed_dims, kernel_size=3, stride=1,padding=1)
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            if layer == 'conv':
                ...
                # hunxi注意力
                # in_channel = query.shape[1]
                # # cbam = BAM_Block(channal) # CBAM、BAM # ECA-Net(Efficient Channel Attention)
                # # query = cbam(query)
                # dev = query.device
                # sa = ShuffleAttention3D(channel=in_channel, G=8).to(dev) # SA-Net(Shuffle Attention)
                # query = sa(query)
