...
class DepthwiseSeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=False):
        ...
        self.depthwise = nn.Conv2d(
            ...
            padding=padding,
            dilation=dilation,
            groups=in_channels,  # 关键：groups=in_channels 表示每个输入通道单独卷积
            bias=bias
        )
...
class DepthwiseSeparableDeconv3D(nn.Module):
...
class OccHead(nn.Module): 
    ...
    def _init_layers(self):
            ...
            if stride > 1:
                if not DSConv:
                    ...
                    upsample_layer = build_upsample_layer(
                        ...
                        stride=upsample_strides[i],
                        groups=out_channel)
                    # # DSDeconv3d 三维深度可分离反卷积 # 20250221
                    # upsample_layer = DepthwiseSeparableDeconv3D(in_channels=in_channels[i], 
                    #                                             out_channels=out_channel, 
                    #                                             kernel_size=upsample_strides[i], 
                    #                                             stride=upsample_strides[i], 
                    #                                             padding=0
                ...
                if not DSConv:
                    ...
                    upsample_layer = build_conv_layer(
                        ...
                        padding=1,
                        groups=out_channel)
                # # DSConv3d 三维深度可分离卷积 # 20250221
                # upsample_layer = DepthwiseSeparableConv3d(in_channels=in_channels[i], 
                #                                           out_channels=out_channel, 
                #                                           kernel_size=3, 
                #                                           stride=1,
                #                                           padding=1)
        ...
        # 20250305 # Faster OccHead
        self.deblocks2 = nn.ModuleList()
        in_channels2 = [512,17,256,17,128,17]
        out_channels2 = [17,17,17,17,17,17]
        for i, out_channel in enumerate(out_channels2):
            stride = upsample_strides[i] # upsample_stride=[1,2,1,2,1,2]
            if stride > 1:
                norm_cfg=dict(type='GN', num_groups=1, requires_grad=True)
                upsample_layer = DepthwiseSeparableDeconv3D(in_channels=in_channels2[i], 
                                                            out_channels=out_channel, 
                                                            kernel_size=upsample_strides[i], 
                                                            stride=upsample_strides[i], 
                                                            padding=0
                ...
                norm_cfg=dict(type='GN', num_groups=1, requires_grad=True)
                upsample_layer = DepthwiseSeparableConv3d(in_channels=in_channels2[i], 
                                                          out_channels=out_channel, 
                                                          kernel_size=3, 
                                                          stride=1,
                                                          padding=1)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # # 20250305 Faster OccHead
        # for i in range(len(self.deblocks2)):
        #     if i in [0]:
        #         result = self.deblocks2[i](result)
        #         outputs.append(result)
        #     elif i in [1,3]:
        #         result = self.deblocks2[i](result)
        #     elif i in [2,4]:
        #         volume_embed_temp = volume_embed_reshape.pop()
        #         result = self.deblocks2[i](volume_embed_temp)+result
        #         outputs.append(result)
        #     elif i in [5]:
        #         result = self.deblocks2[i](result)
        #         outputs.append(result)
    ...
    def loss(self,
        ...
        if not self.use_semantic:
            ...
            for i in range(len(preds_dicts['occ_preds'])):
                ...
                loss_occ_i = loss_occ_i * ((0.5)**(len(preds_dicts['occ_preds']) - 1 -i)) # 原版
